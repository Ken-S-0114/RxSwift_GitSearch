...
final class RepositoryListController: UIViewController {
    ...
    private func setupRx() {
         ...
         *
         * (フロー1) → 検索バーでの入力値の更新が「データ取得のトリガー」になるので、ViewModel側に定義した fetchRepositories() メソッドが実行される
         *          （このタイミングで　lazy var repositories が動く）
         * (フロー2) → fetchRepositories()メソッドが実行後は、ViewModel側に定義したメンバ変数 repositories に値が格納される
         */
        repositoriesViewModel = RepositoriesViewModel(withNameObservable: searchBarText)
        ...
        repositoriesViewModel
            .repositories
            .drive(repositoryListTableView.rx.items) { (tableView, indexPath, repository) in
                let cell = tableView.dequeueReusableCell(withIdentifier: "RepositoryCell", for: IndexPath(row: indexPath, section: 0))
